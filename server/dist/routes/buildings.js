...
router.get('/:walletAddress', async (req, res) => {
    try {
        ...
        const users = await (0, client_1.sql) `SELECT id FROM users WHERE wallet_address = ${walletAddress}`;
        if (users.length === 0) {
...
router.post('/build', async (req, res) => {
    try {
        ...
        const users = await (0, client_1.sql) `SELECT * FROM users WHERE wallet_address = ${walletAddress}`;
        if (users.length === 0) {
            return res.status(404).json({ error: 'Player not found' });
        }
        const user = users[0];
        const existingBuildings = await (0, client_1.sql) `
    ...
    `;
        ...
        // Deduct points
        await (0, client_1.sql) `UPDATE users SET points = points - ${cost} WHERE id = ${user.id}`;
...
router.post('/storage/deposit', async (req, res) => {
    try {
        ...
        const user = users[0];
        const storages = await (0, client_1.sql) `
    ...
    `;
        ...
        await (0, client_1.sql) `UPDATE users SET points = points - ${amount} WHERE id = ${user.id}`;
        await (0, client_1.sql) `UPDATE buildings SET stored_points = stored_points + ${amount} WHERE id = ${storage.id}`;
        return res.json({
            success: true,
            deposited: amount,
