...
function formatMcap(value: number | null | undefined) {
...
async function fetchDexScreenerMarket(contractAddress: string, chainId: string) {
  const urls = [
  ...
  for (const url of urls) {
    ...
    const data = await response.json() as { pairs?: any[] } | any[];
